...
insert into invoice_item (invoice_id, item_id, quantity)
    ...
    select first_name from customer,invoice,invoice_item where customer.id=invoice.customer_id AND invoice.id = invoice_item.invoice_id;
    select name,price,id, quantity , price*quantity as 'total' 
    from item,customer,invoice,invoice_item where
    item.id = invoice_item.item_id AND customer.id = invoice.customer_id AND invoice.id = invoice_item.invoice_id;


select invoice.id,customer.first_name,customer.last_name,name,price,customer.id, quantity , price*quantity as 'total' 
    from item,customer,invoice,invoice_item where
    item.id = invoice_item.item_id AND customer.id = invoice.customer_id AND invoice.id = invoice_item.invoice_id;


select item.id from item,invoice,invoice_item ,customer 
